...
table::rows query::integer_match(int query_value,
    ...
    auto integer_match_view =
        vw_integer_match(query_value, targets_vw);
...
table::rows query::boolean_match(bool query_value,
    ...
    auto boolean_match_view =
        vw_boolean_match(query_value, targets_vw);
...
table::rows query::floating_match(float query_value,
    ...
    auto floating_match_view =
        vw_floating_match(query_value, targets_vw);
...
table::rows query::floating_match(const string& query_value,
    ...
    return floating_match(qf, rows_to_query);
}

table::rows query::geo_query_match(const coordinate& coord,
                                   table::opt_rows rows_to_query) {
    table::rows targets = rows_to_query ? *rows_to_query : t.rows_;
    auto targets_vw = views::all(targets);
    ...
    auto geo_coordinate_match_view =
        vw_geo_query_match(coord, targets);
    auto result = ranges::to<table::rows>(geo_coordinate_match_view);
    return result;
}

table::rows query::geo_query_match(const string& query_value,
                                   table::opt_rows rows_to_query) {
    println(stderr, "geo_query_match: query_value = {}", query_value);
    table::rows targets = rows_to_query ? *rows_to_query : t.rows_;
    // auto targets_vw = views::all(targets);
    const coordinate coord = make_coordinate(query_value);
    return geo_query_match(coord, targets);
}

// auto vw_tags_match(table& t, const string& col_name,
//                    const vector<string>& query_values,
//                    ranges::ref_view<table::rows> targets);

// table::rows tags_match(table& t, const string& col_name,
//                        const string& query_value,
//                        table::opt_rows rows_to_query) {
//     const vector<string> query_values({query_value});
//     return tags_match(t, col_name, query_values, rows_to_query);
// }

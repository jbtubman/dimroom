...
TEST_F(parse_utils_test_fixture, ParseUtilsSplitRow0) {
    const string input{""};
...
TEST_F(parse_utils_test_fixture, ParseUtilsSplitRow1) {
...
TEST_F(parse_utils_test_fixture, ParseUtilsSplitRow2) {
...
TEST_F(parse_utils_test_fixture, ParseUtilsSplitRow3) {
    const string input{
        R"(600,"36° 00' N, 138° 00' E",,Asia,,,,"""Mt Fuji, Fog""")"};
    const vector<string> expected({"600"s, R"("36° 00' N)", R"( 138° 00' E")",
                                   ""s, "Asia"s, ""s, ""s, ""s, R"("""Mt Fuji)",
                                   R"( Fog""")"});
    const vector<string> result = split_row(input);
    EXPECT_TRUE(result == expected);
}

TEST_F(parse_utils_test_fixture, ParseUtilsCombineDecimalCoordinateFields) {
    const string input_s{R"(1200,"51.05011, -114.08529",Yes)"};
    const vector<string> input = split_row(input_s);
    const string expected{
        "1200"s + ","s + parse_utils_test_fixture::csv_dec_cord_start +
        comma_substitute + parse_utils_test_fixture::csv_dec_cord_end + ","s +
        "Yes"s};
...
TEST_F(parse_utils_test_fixture, ParseUtilsCombineDMSCoordinateFields) {
    const string input_s{R"(600,"36° 00' N, 138° 00' E",,Asia)"};
    const vector<string> input = split_row(input_s);
    const string expected =
        "600"s + ","s + parse_utils_test_fixture::csv_dms_cord_start +
        comma_substitute + parse_utils_test_fixture::csv_dms_cord_end + ","s +
        ""s + ","s + "Asia"s;
    const string result = combine_coordinate_fields(input);
    EXPECT_TRUE(result == expected);
}

TEST_F(parse_utils_test_fixture, ParseUtilsCombineTagFields0) {
    // Empty tag list.
...
TEST_F(parse_utils_test_fixture, ParseUtilsCombineTagFields1) {
    // Empty tag list.
...
TEST_F(parse_utils_test_fixture, ParseUtilsCombineTagFields3) {
    const vector<string> input{"Yes"s,       ""s,          "32",
                               "Y",          "Flames",     R"("""Urban)",
                               " B&W Photo", R"( Dusk""")"};
    const string expected = R"(Yes,,32,Y,Flames,"""Urban)" + comma_substitute +
                            " B&W Photo" + comma_substitute + R"( Dusk""")";
    const string result = combine_tag_fields(input);
    EXPECT_TRUE(result == expected);
}

TEST_F(parse_utils_test_fixture, ParseUtilsFixQuotedFields) {
    string row_s(parse_utils_test_fixture::sample_row_3);
    auto result = fix_quoted_fields(row_s);
    EXPECT_TRUE(valid_csv_decimal_coord == result[6]);
    EXPECT_TRUE(valid_tag_sample_row_3 == result[12]);
...
TEST_F(parse_utils_test_fixture, ParseUtilsDetermineCellDataType) {
    for (parse_utils_test_fixture::result_expected_t string_type_pair :
